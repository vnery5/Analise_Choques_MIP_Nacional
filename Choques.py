...
if __name__ == '__main__':
    ...
    # If decided to use APF's matrices, remember to specify nDimension = 0 and nSectorsFile = 67 on line 75
    ...
    sFileShock = "VetorChoqueRefino.xlsx"
    # sFileShock = "AgregRevisão.xlsx"
    sSheetDemandShock = "ChoqueDemandaRefinoPositivo"
    ...
    if shockDemand:
        ...
        # Adding up to find new final demand and shaping it as a (n x 1) matrix
    ...
    ## Transposing added value and payment sector tables (in order for the sectors to be in the rows)
    # Added Value
    mCoefAV = mCoefAV.T
    ...
    vImpactSheet_Index = ["VPB", "VA Setorial", "Impostos s/Produtos (CI)",  # "Impostos s/Produtos (DF)", "PIB",
                          "Empregos", "EOB", "RM", "EOB+RM", "Remunerações", "Outros Impostos"]
    ...
    mImpactSheet = np.vstack((vProduction, vAV_Sectors, vTaxesProductsIC, vEmp,  # vTaxesProductsFD, vGDP,
                              vEOB, vRM, vEOB_RM, vRem, vOtherTaxesProd))
    ...
    if shockSupply and shockDemand:
        ...
        vNameSheets = ["MIP", "Choque_Demanda", f"Impacto 'PIB' (VA Setorial){AggregIndicator}",
                       f"Impacto VPB{AggregIndicator}", "Resumo Impactos",
                       "Coeficientes VA e SP", "Componentes VA - MIP", "Componentes VA - Choque",
                       "Delta Componentes VA"]
        vDataSheets = [dfMIP, dfShockDemand, dfSheetV, dfSheetX, dfImpactSheet,
                       dfCoef_SP_AV, dfOld_SP_AV, dfNew_SP_AV, dfDelta_SP_AV]
    else:
        vNameSheets = ["MIP", "Choque_Oferta", f"Impacto 'PIB' (VA Setorial){AggregIndicator}",
                       f"Impacto VPB{AggregIndicator}", "Resumo Impactos",
                       "Coeficientes VA e SP", "Componentes VA - MIP", "Componentes VA - Choque",
                       "Delta Componentes VA"]
        vDataSheets = [dfMIP, dfShockSupply, dfSheetV, dfSheetX, dfImpactSheet,
                       dfCoef_SP_AV, dfOld_SP_AV, dfNew_SP_AV, dfDelta_SP_AV]
